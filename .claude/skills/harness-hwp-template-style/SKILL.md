...
## 절차 (읽기 전용 — 계획만)
> 📄 대상/레퍼런스 **HWP·HWPX 내용 읽기 = kordoc(MCP) 추출**(구조 파악용; format.yaml `engine_routing.read`). 표 양식 **적용(쓰기)**은 hwp-table-style(pyhwpx/한컴) 유지. 정본 `_writing-core/document-extraction.md`.
1. `format_profile`(`_format-profiles/hwp/<name>/format.yaml`) Read → engine·engine_routing·reference_template 확인.
2. 대상/레퍼런스 표 구조 파악(kordoc 추출 또는 inspect_tables 위임 계획).
