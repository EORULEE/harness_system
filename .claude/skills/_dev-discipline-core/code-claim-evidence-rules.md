...
correctness-critical 주장만: x-에이전트/Codex 에게 *유일 임무 = "그 값을 생산하는 함수를 직접 열어 읽고 주장을 반박하라"*. 다른 모델 = 다른 블라인드. 비용 때문에 결정적 주장에만. (this-project 파일럿은 1·2층; 3층은 codex exec 적대검토(CLI) 명시 호출로.)
