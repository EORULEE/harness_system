...
- research → research-agent + multi_research.py(chatgpt) + 교차반박 + obsidian/zotero
...
- knowledge-promotion → claim-evidence-audit → codex exec 적대검토(CLI) → 재검증 → (승인) 승격
...
- **cross-model**: `codex exec` 적대검토(CLI 직접, gpt-5.6-sol). **Codex 결과 ≠ 정본** — Claude 가 source 로 재검증. 유료 → human-gated 동반.
