...
> 특정 프로젝트/도메인 하드코딩 없이 IITP·fusion 등 어디서나. (files_origin 등 페어 없는 프로젝트는 graceful skip.)

**하이브리드 2단계:**
1. **결정적 추출** (스크립트) — `python3 <distill> --project <프로젝트 루트>`
   - `<distill>` 해석: 글로벌 설치 `~/.claude/lib/distill_domain_from_agents.py` 우선, 없으면 repo `scripts/distill_domain_from_agents.py`(files_origin dev). 순수 stdlib·cwd 무관.
