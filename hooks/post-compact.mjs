#!/usr/bin/env node
/**
 * post-compact.mjs — PostCompact hook (경량 queue writer).
 * Memory Continuity v2: compact summary(있으면) + metadata 를 queue 에 atomic 기록 후 즉시 종료.
 *  - transcript 전체 분석 X, LLM X, 네트워크 X.
 *  - compact_summary 는 bounded(4KB) + light-mask 후 저장(raw secret 저장 금지).
 *  - 어떤 예외에도 블로킹하지 않음(exit 0).
...
if (ev && ev !== "PostCompact") process.exit(0);
...
const rawSummary =
  j.compact_summary || j.summary || j.compactSummary ||
  (j.hookSpecificOutput && (j.hookSpecificOutput.summary || j.hookSpecificOutput.compactSummary)) || "";
...
const event = {
  ...
  reason: maskLight(String(j.trigger || j.reason || "post_compact")).slice(0, 200),
  compact_summary: maskLight(String(rawSummary)).slice(0, 4000), // bounded + masked
};

const sh = createHash("sha256").update(String(event.session_id || "nosession")).digest("hex").slice(0, 12);
try {
  mkdirSync(qdir, { recursive: true });
  // dedupe: 같은 session+event 의 pending compact 제거(최신 1개만 유지)
  try {
    for (const f of readdirSync(qdir)) {
      if (f.endsWith(`-post_compact-${sh}.json`)) { try { unlinkSync(join(qdir, f)); } catch {} }
    }
  } catch {}
  const base = `${event.timestamp.replace(/[:.]/g, "")}-post_compact-${sh}.json`;
